-- Script de configuración corregido para Supabase
...
-- Políticas de seguridad simplificadas para usuarios
-- Permitir que todos vean usuarios (para login)
CREATE POLICY "Allow all users to view users table" ON users
    FOR SELECT USING (true);

-- Permitir que todos inserten usuarios (registro)
CREATE POLICY "Allow all users to insert users" ON users
    FOR INSERT WITH CHECK (true);

-- Permitir que todos actualicen usuarios (para actualizar last_login)
CREATE POLICY "Allow all users to update users" ON users
    FOR UPDATE USING (true);

-- Políticas de seguridad para resultados de poker
-- Permitir que todos vean resultados (para análisis)
CREATE POLICY "Allow all users to view poker results" ON poker_results
    FOR SELECT USING (true);

-- Permitir que todos inserten resultados
CREATE POLICY "Allow all users to insert poker results" ON poker_results
    FOR INSERT WITH CHECK (true);

-- Permitir que todos actualicen resultados
CREATE POLICY "Allow all users to update poker results" ON poker_results
    FOR UPDATE USING (true);

-- Permitir que todos eliminen resultados
CREATE POLICY "Allow all users to delete poker results" ON poker_results
